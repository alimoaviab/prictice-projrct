/**
 * Ported 1:1 from old-app/school-app/services/academic-year-context.ts.
 *
 * Stores the user-selected academic year. The active year is also baked into
 * the JWT (the server source of truth), but the client mirrors it here so it
 * can be sent on every request via the `x-academic-year-id` header.
 */
...
function scopedKey(schoolId?: string) {
  return schoolId
    ? `${ACADEMIC_YEAR_STORAGE_KEY}:${schoolId}`
...
export function getSelectedAcademicYearId(): string | undefined {
  ...
  const scoped = schoolId
    ? window.localStorage.getItem(scopedKey(schoolId))
    : null;
