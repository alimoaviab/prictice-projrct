...
func makeInvoiceNo(studentID, month string, year int) string {
	var randomPart int64
	n, err := rand.Int(rand.Reader, big.NewInt(1<<62-1))
	if err != nil {
		randomPart = time.Now().UnixNano() // fallback
	} else {
		randomPart = n.Int64()
	}
	src := fmt.Sprintf("%s:%s:%d:%d:%d", studentID, month, year, time.Now().UnixNano(), randomPart)
	h := sha1.Sum([]byte(src))
...
func makeReceiptNo() string {
	var suffix string
	n, err := rand.Int(rand.Reader, big.NewInt(0xFFFFFF))
	if err != nil {
