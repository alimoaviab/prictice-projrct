...
const eventsListCacheTTL = 60 * time.Second
...
func listCacheKey(schoolID, query string) string {
	...
	h := sha1.Sum([]byte(src))
	return fmt.Sprintf("events:list:%s:%s", schoolID, hex.EncodeToString(h[:])[:16])
}
...
func (h *Handler) invalidateList(r *http.Request, schoolID string) {
	...
	_, _ = h.Cache.DelPattern(r.Context(), fmt.Sprintf("events:list:%s:*", schoolID))
}
...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	cacheKey := listCacheKey(ctx.SchoolID, q.Encode())
	if h.Cache != nil && h.Cache.Available() {
	...
	if err != nil {
		api.WriteResult(w, api.Fail("INTERNAL", "Failed to encode events.", 500, nil))
		return
	}
	w.Header().Set("Content-Type", "application/json")
	if h.Cache != nil && h.Cache.Available() {
	...
	if h.Cache != nil && h.Cache.Available() {
		_ = h.Cache.Set(r.Context(), cacheKey, bytes, eventsListCacheTTL)
	}
}
