...
export function useTeachers(
  academyYearIdOrOptions?: string | TeacherListOptions
) {
  const { schoolId, academicYearId: ctxYear, role } = useTenantContext();

  const opts: TeacherListOptions = typeof academyYearIdOrOptions === 'string' || academyYearIdOrOptions === undefined
    ? {}
    : (academyYearIdOrOptions as TeacherListOptions);

  const effectiveYear = typeof academyYearIdOrOptions === 'string' ? academyYearIdOrOptions : ctxYear;
...
export function useTeacher(teacherId: string) {
  ...
  return useQuery({
    queryKey: [...TEACHERS_QUERY_KEY, schoolId, academicYearId, 'detail', teacherId],
    queryFn: async () => {
