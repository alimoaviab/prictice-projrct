...
type StudentListOptions = {
  ...
  limit?: number;
  /**
   * If false (default), the legacy array shape is returned for backwards
   * compatibility with existing UIs. Pass `paginated: true` when you want
...
/**
 * Backwards-compatible students hook.
 *
 * Existing call sites: `useStudents(classId, academicYearId)` keep working
 * exactly as before — they receive a plain array.
 *
 * New call sites can pass an options object with `paginated: true` to opt
 * into server-side pagination, search, and limit selection.
 */
export function useStudents(
  ...
  const opts: StudentListOptions = typeof classIdOrOptions === 'string' || classIdOrOptions === undefined
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data || (opts.paginated ? { items: [], total: 0, page: 1, limit: 25, pages: 1 } : []);
    },
