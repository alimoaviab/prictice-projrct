...
interface ConfirmModalProps {
    isOpen: boolean;
    title: string;
    /** Primary message describing the action's effect. */
    message: string;
    /** Optional small note shown beneath the message in muted text. */
    note?: string;
    /** Optional named target (e.g. student name, class name) shown in a chip. */
    itemName?: string;
    confirmLabel?: string;
    cancelLabel?: string;
...
const variantTokens: Record<ConfirmVariant, { iconBg: string; iconColor: string; iconName: string; button: string }> = {
    danger: {
        iconBg: "bg-red-50",
...
export function ConfirmModal({
    ...
    const finalConfirmLabel = confirmLabel ?? (confirmVariant === "danger" ? "Delete" : "Continue");
        ...
        >
            ...
            >
                <div className="flex items-start gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${tokens.iconBg}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p id="confirm-modal-description" className="text-sm text-slate-600 mt-1.5 leading-relaxed break-words">
                            {message}
                        </p>
                        {itemName && (
