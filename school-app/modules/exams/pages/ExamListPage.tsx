...
export function ExamListPage({ filters }: { filters?: { class_id?: string; subject?: string } }) {
  ...
  const columns: DataTableColumn<ExamRow>[] = [
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{row.subject}</p>
          </div>
        </div>
      ),
      sortable: true,
    ...
    },
    {
      key: "marks",
      label: "Max Marks",
      render: (row) => <span className="text-[11px] font-black text-slate-700">{row.max_marks} Pts</span>,
    },
    {
      key: "status",
      label: "Status",
      render: (row) => (
        <Badge variant={row.status === "completed" ? "success" : row.status === "scheduled" ? "primary" : "gray"} className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5">
          {row.status}
        </Badge>
      ),
    },
                  ...
                  <div className="p-5">
                    ...
                    <div className="mb-6">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-lg font-black text-slate-900 tracking-tight group-hover:text-blue-600 transition-colors">{exam.title}</h3>
                        <Badge variant={exam.status === "completed" ? "success" : exam.status === "scheduled" ? "primary" : "gray"} className="text-[8px] font-black uppercase tracking-widest px-1.5 py-0">
                          {exam.status}
                        </Badge>
                      </div>
                      <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">{exam.subject}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      <div className="bg-slate-50/50 rounded-xl p-2.5 border border-slate-100/50">
                        ...
                        <p className="text-[10px] font-bold text-slate-700 truncate">{exam.class_name || "Unassigned"}</p>
                      </div>
                      <div className="bg-slate-50/50 rounded-xl p-2.5 border border-slate-100/50">
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Max Marks</p>
                        <p className="text-[10px] font-bold text-slate-700">{exam.max_marks} Pts</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{exam.starts_at}</span>
                      </div>
                    </div>
                  </div>
