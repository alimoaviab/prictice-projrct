...
export default function ParentStudentProfilePage() {
    ...
    useEffect(() => {
        ...
        void run(async () => {
            const result = await serviceRequest<ProfileResponse>(`/api/parent/student-info?student_id=${selectedChild.student_id}`);
    ...
    if (childLoading || state.status === "idle" || state.status === "loading") {
        return (
            <SchoolShell eyebrow="Parent Portal" title="Student Profile">
    ...
    if (!selectedChild) {
        return (
            <SchoolShell eyebrow="Parent Portal" title="Student Profile">
                <DataState variant="empty" title="No Student Selected" message="Please select a student to view their profile details." />
            </SchoolShell>
        );
    }
    ...
    return (
        <SchoolShell eyebrow="Parent Portal" title={`Profile: ${selectedChild.student_name}`}>
